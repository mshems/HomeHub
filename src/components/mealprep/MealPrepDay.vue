...
<template>
  <div v-if="!!date" class="q-px-md q-py-xs">
    ...
    <div v-if="pending" class="q-mb-sm">
      <q-skeleton />
    </div>

    <div v-else-if="!day" class="q-mb-sm q-pa-sm row justify-center items-center text-muted">

        <!-- <q-icon name="mdi-pot-mix" size="sm" class="q-mr-xs q-pb-xs"/> -->
        <q-icon name="mdi-food" size="sm" class="q-mr-sm q-pb-xs"/>
        <div style="font-size: 1rem;">No Meals Planned</div>
    </div>

    <div v-else class="q-mb-sm">
      <div class="row q-gutter-xs q-mb-sm">
        <template v-for="meal in day.meals" :key="meal.id">
          <meal-card :meal="meal" class="col-auto"/>
        </template>
        <q-btn icon="mdi-pot-steam" outline color="recipes" class="col-auto" />
      </div>

      <div v-if="day.notes">
        <q-card>
          <q-card-section>
            <div class="text-mono row items-center"><q-icon name="mdi-note" class="q-mr-xs"/>Notes</div>
            <div class="text-muted text-mono">{{ day.notes }}</div>
          </q-card-section>
        </q-card>
      </div>
    </div>
